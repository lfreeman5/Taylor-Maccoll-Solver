...
coefficients = create_coeffs('results_CP.json', 'cps')

gen_thetas = np.linspace(0,1,30)
gen_machs = np.linspace(1,30,30)
gen_thetas, gen_machs = np.meshgrid(gen_thetas, gen_machs)
gen_cps = evaluate_function(coefficients, [1.4*np.ones_like(gen_thetas), gen_machs, gen_thetas])
...
for mach, m_dict in data.items():
    ...
    thetas = m_dict['thetas']
    cps = m_dict['cps']
    error = (cps - evaluate_function(coefficients, [np.ones(len(thetas))*1.4, np.ones(len(thetas))*mach, np.array(thetas)]))*100
